#!/usr/bin/env bash
# Rename all chapter folders from "ChapterN" to "Chapter N" recursively
# Usage:
#   bash scripts/rename_chapter_folders.sh [ROOT]
# Env:
...
done < <(find "$ROOT" -type d -name 'Chapter[0-9]*' -depth -print0)
